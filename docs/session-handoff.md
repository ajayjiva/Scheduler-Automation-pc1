...
| [`docs/reconcile_exceptions.md`](./reconcile_exceptions.md) | `reconcile_exceptions.py` — Phase 3 writer. Reads active `pc1.scheduleexceptions` rules and overlays them onto `pc1.machineschedule` (`exceptions[]` / `exception_ids[]` / `availability`). Modality matched by FK. Availability rule: reconciler owns in-hours, generator owns out-of-hours. Booked-slot Hard-exception conflict surfaces a warning + exit code 4 (Phase-4-orders-dependent) |
...
| 2 | Port **`reconcile_exceptions.py`** | **Phase 3 IN PROGRESS** — `reconcile_exceptions.py` + `docs/reconcile_exceptions.md` on `feature/pc1-machineschedule-reconciler`. Reads `pc1.scheduleexceptions` (modality FK match), overlays onto `pc1.machineschedule`. Booked-slot Hard conflict surfaces warning + exit code 4 — exercises only once Phase 4 populates `order_id` | Idempotent re-runs (no DB writes when desired state already matches). Default iterates all `is_client=true` facilities; `--facility=NAME` for one. Parallelism / batching ported verbatim from legacy |
